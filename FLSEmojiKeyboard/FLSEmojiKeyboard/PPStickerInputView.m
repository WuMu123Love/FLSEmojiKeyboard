...
static CGFloat const PPStickerTextViewHeight = 44.0;//输入框高度
...
static NSUInteger const PPStickerTextViewMaxLineCount = 5;
static NSUInteger const PPStickerTextViewMinLineCount = 1;
static CGFloat const PPStickerTextViewLineSpacing = 5.0;
static CGFloat const PPStickerTextViewFontSize = 16.0;

static CGFloat const PPStickerTextViewEmojiToggleLength = 1.0;
...
@property (nonatomic, strong) UIView *separatedLine;//分割线
@property (nonatomic, strong) PPButton *emojiToggleButton;//表情和键盘切换按钮
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.exclusiveTouch = YES;//解决多个控件同时响应事件的问题
...
- (CGFloat)heightThatFits
{
    ...
    if (self.keepsPreModeTextViewWillEdited) {
        return PPStickerTextViewHeight;//44.0
    } else {
        ...
        CGFloat minHeight = [self heightWithLine:PPStickerTextViewMinLineCount];//3
        CGFloat maxHeight = [self heightWithLine:PPStickerTextViewMaxLineCount];//6
...
- (CGRect)frameTextView
{
    ...
    CGFloat minX = (self.textView.isFirstResponder ? PPStickerTextViewTextViewLeftRightPadding : PPStickerTextViewTextViewUnfocusLeftRightPadding);//16:5
//    CGFloat width = self.bounds.size.width - (2 * minX);
    CGFloat width = self.bounds.size.width - minX - PPStickerTextViewTextViewTopMargin -PPStickerTextViewToggleButtonLength * 2;
    CGFloat height = 0;
    if (self.keepsPreModeTextViewWillEdited) {
        height = CGRectGetHeight(self.bounds) - 2 * PPStickerTextViewTextViewTopMargin;//10
    } else {
        height = CGRectGetHeight(self.bounds) - PPStickerTextViewTextViewTopMargin - PPStickerTextViewTextViewBottomMargin - PPStickerTextViewEmojiToggleLength;//10,10,48
    }
    if (height < 0) {
        height = self.bounds.size.height;
    }
    NSLog(@"bounds:%f",CGRectGetHeight(self.bounds));
    NSLog(@"height:%f",height);
    return CGRectMake(minX, PPStickerTextViewTextViewTopMargin, width, height);//10
}

- (CGRect)frameSeparatedLine
{
    
    return CGRectMake(0, CGRectGetHeight(self.bounds) - PPStickerTextViewEmojiToggleLength, self.bounds.size.width, PPOnePixelToPoint());//48
}

- (CGRect)frameEmojiToggleButton
{
    
     return CGRectMake(CGRectGetWidth(self.bounds) - PPStickerTextViewToggleButtonLength-PPStickerTextViewTextViewLeftRightPadding, CGRectGetHeight(self.bounds)-PPStickerTextViewToggleButtonLength-PPStickerTextViewTextViewTopMargin, PPStickerTextViewToggleButtonLength, PPStickerTextViewToggleButtonLength);//16,(48,24),24,24
    
//    return CGRectMake(PPStickerTextViewTextViewLeftRightPadding, CGRectGetHeight(self.bounds) - (PPStickerTextViewEmojiToggleLength + PPStickerTextViewToggleButtonLength) / 2, PPStickerTextViewToggleButtonLength, PPStickerTextViewToggleButtonLength);//16,(48,24),24,24
}
